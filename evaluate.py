...
def evaluate():
  ...
  for i in range(sequences):
    image_path = os.path.join(FLAGS.data_dir, FLAGS.case, image_filenames_list[i])
    image_paths.append(image_path)
    image = sitk.ReadImage(image_path)
    images.append(image)
    images_tfm.append(image) # maintaining two separate lists just to be safe, I should get better at python
  
  true_label = sitk.ReadImage(os.path.join(FLAGS.data_dir, FLAGS.case, FLAGS.label_filename))
  ...
  writer.SetFileName(os.path.join(FLAGS.data_dir, FLAGS.case, 'prob_centernet.nii.gz'))
